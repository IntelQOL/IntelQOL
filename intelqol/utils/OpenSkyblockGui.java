...
import intelqol.commands.Backpack;
import intelqol.commands.LobbySwap;
...
public class OpenSkyblockGui {
    ...
    public void guiDraw(GuiScreenEvent.BackgroundDrawnEvent event) {
        if (!intelqol.commands.Wardrobe.openWardrobe || openingWardrobe) return;
        new Thread(() -> {
            try {
                        ...
                        if (chestName.contains("Pets")) {
                            ...
                            for (i = 0; i < invSlots.size(); i++) {
                                ...
                                if (StringUtils.stripControlCodes(invSlots.get(i).getStack().getDisplayName()).equals("Wardrobe")) {
                                    clickSlot(invSlots.get(i));
                                    if (intelqol.commands.Wardrobe.slot == 0)
                                        intelqol.commands.Wardrobe.openWardrobe = false;
                                }
                            }
                            ...
                            if (intelqol.commands.Wardrobe.slot != 0) {
                                ...
                                for (i = 0; i < invSlots.size(); i++) {
                                    if (!invSlots.get(i).getHasStack()) continue;
                                    String slotName = "Slot " + intelqol.commands.Wardrobe.slot + ":";
